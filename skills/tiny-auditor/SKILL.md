...
- Vulnerable Occurrences are be bullets with (usually) github links with exact line references and commit path to the vulnerable sections of the code that directly create the vulnerability. If the audited item is not a code with direct gitlink, specifiy all the affected endpoints, or whatever it is insetad. If an evidence (e.g. api key name) is preset, prioritize it in reading of the bullets (e.g. * `found_secret` (Line 485)) - where line is a hyperlink, putting more priority on the key name itsefl which is easier on the fixing team to understand on a quick read.
